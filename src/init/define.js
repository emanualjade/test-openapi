...
const defineTasks = function({ config: { tasks, runTask, ...config }, plugins, errors }) {
  ...
  describe(DESCRIBE_TITLE, () => defineAllTasks({ tasks, runTask, handlers }))
}

const DESCRIBE_TITLE = 'Tasks'

const defineAllTasks = function({ tasks, runTask, handlers }) {
  tasks.forEach(task => defineTask({ task, runTask, handlers }))
}
...
const defineTask = function({ task, task: { taskKey }, runTask, handlers }) {
  ...
  it(taskKey, () => runTask(task, handlers), 0)
}
...
const getHandlers = function({
  ...
  readOnlyArgs.runTask = task => originalRunTask(task, handlers)
