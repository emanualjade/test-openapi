...
const addUrl = function({ call, call: { request, rawRequest } = {} }, { config, helpers }) {
  if (call === undefined) {
    return
  }

  const url = getFullUrl({ rawRequest, config, helpers })
...
const getFullUrl = function({ rawRequest, config, helpers }) {
  const url = getServer({ rawRequest, config, helpers })
